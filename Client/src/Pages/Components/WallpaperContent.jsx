...
function wallpaperContent() {
  ...
  const pagination = async () => {
    const data = categories.slice(initialValue, lastValue);
  ...
  const showCategories = async () => {
    axios.get(`http://localhost:3000/api/admin/wallpapers`).then((res) => {
      ...
      if (res.status === 200) {
        ...
        setcategories(res.data.wallpapers);
      } else {
  ...
  const handleCount = () => {
    setinitialValue(initialValue + 9);
    setlastValue(initialValue + 9 + 9);
    pagination();
    showCategories();
  ...
  const handleSave = (categoryData) => {
    console.log("here is your data", categoryData);
    if (selectedCategories) {
      ...
      axios
        ...
        .then((res) => {
          ...
          if (res.status === 200) {
            setcategories((prev) =>
              prev.map((cat) =>
                cat.id === selectedCategories.id ? res.data.getwall : cat
              )
            );
      ...
      axios
        ...
        .catch((error) => {
          ...
          return res
            .status(500)
            .json({ message: " Error while adding", error });
        });
    }
  };
            ...
            <div class="inline-block min-w-full align-middle">
              <div class="overflow-hidden shadow">
                {categories.length  === 0  ?    <div  className="flex justify-center items-center bg-gray-800 divide-y divide-gray-200 w-full h-screen">
                  ...
                  </div> :   <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
              ...
              <thead class="bg-gray-100 dark:bg-gray-700 w-screen">
                <tr className="bg-gray-100 dark:bg-gray-800">
                  ...
                  >
                    ...
                    {categories.slice(initialValue, lastValue).map((w) =>

                    
                    
                       <tbody class="bg-gray-800 divide-y divide-gray-200   ">
                        <tr class="hover:bg-gray-100 dark:hover:bg-gray-700" key={w.categoryId}>
                          <td class="w-17 p-4">
                            <div className="flex items-center">
                              <img
                                
                                src={w.wallpaper_image_url}
                                alt={w.wallpaper_title}
                          ...
                          <td class="p-4 text-sm font-normal text-gray-500 whitespace-nowrap dark:text-gray-400">
                            <div class="text-base font-semibold text-gray-900 dark:text-white">
                              {w ? w.categoryId : <>no category found</>}
                            </div>
                          </td>
          ...
          <div className="bg-gray-800 p-5 flex justify-end">
            {initialValue > 0 && (
              <button
                ...
                class="text-white bg-blue-700 hover:bg-primary-800 m-2 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800"
              >
                Prev
              </button>
            )}
            {categories.length > lastValue && (
              <button
                onClick={handleCount}
                class="text-white bg-blue-700 hover:bg-primary-800 m-2 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800"
              >
